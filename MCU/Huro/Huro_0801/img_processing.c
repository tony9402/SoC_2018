...
U32 Find_Color(U16 Color, xy *Inpos, U32 *Count, range *Range)//색,동적할당,변수(몇개받을건지),넒이
{
	short i, j;
	U16 *input = (U16*)malloc(43200);
	read_fpga_video_data(input);
	...
	memcpy(Inpos, position, sizeof(position));
	*Count = circle;
	draw_fpga_video_data_full(input);
	flip();
...
void Gate(int *StopCnt)
{
	...
	Find_Color(0x001F, Gate_pos, &Count, &Range);
